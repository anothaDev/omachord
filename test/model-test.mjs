...
assert.equal(model.uniqueId("Constructor", []), "constructor", "An inherited object property is not an occupied routine ID")
assert.equal(model.uniqueId("Constructor", [{ id: "constructor" }]), "constructor-2", "An actual occupied routine ID still gets a suffix")
...
for (const ssid of [" ", " Studio ", " ".repeat(32), "x".repeat(32), "é".repeat(16), " " + "é".repeat(15) + " ", "界".repeat(10) + "é", "😀".repeat(8)])
  assert.equal(model.validateCondition({ type: "wifi", ssids: [ssid] }), "", `SSID within 32 UTF-8 bytes must be accepted: ${JSON.stringify(ssid)}`)
for (const ssid of ["é".repeat(16) + "x", " " + "é".repeat(16), "界".repeat(11), "😀".repeat(8) + "x", "", " ".repeat(33), "x".repeat(33)])
  assert.equal(model.validateCondition({ type: "wifi", ssids: [ssid] }), "Wi-Fi network names must be 1 to 32 UTF-8 bytes", `Empty or over-32-byte SSID must be rejected: ${JSON.stringify(ssid)}`)
assert.equal(model.validateCondition({ type: "wifi", ssids: [" Studio ", "Studio", "   "] }), "", "Distinct exact SSIDs must remain valid")
assert.equal(model.validateCondition({ type: "wifi", ssids: ["   ", "   "] }), "Wi-Fi network names must be unique", "Exact whitespace duplicates must be rejected")
